...
struct top_tree_node {
	...
	void update() {
		if (is_path) {
			if (!is_vert) {
				tot = val;
				tot = std::gcd(tot, c[0]->tot);
				tot = std::gcd(tot, c[1]->tot);
			}
		} else {
		}
	}
...
int main() {
	...
	for (int case_num = 1; case_num <= T; case_num ++) {
		...
		for (int e = 0; e < N-1; e++) {
			int x, y, l; int64_t a; cin >> x >> y >> l >> a; x--, y--;
			edges[e].V = {x,y};
			edges[e].L = l;
			edges[e].A = a;
		}
		sort(edges.begin(), edges.end(), [&](const auto& a, const auto& b) { return a.L < b.L; });
		for (int e = 0; e < N-1; e++) {
			auto [x, y] = edges[e].V;
			nodes[N+e].val = 0;
			link(&nodes[N+e], &nodes[x], &nodes[y]);
		}
		...
		auto it = edges.begin();
		for (auto [w, c, q] : queries) {
			while (it != edges.end() && w >= it->L) {
				auto [x, y] = it->V;
				auto n = get_path(&nodes[x], &nodes[y]);
				n->val = it->A;
				n->update();
				n->downdate();
				n->update_all();
			...
			ans[q] = get_path(&nodes[0], &nodes[c])->tot;
		}
